...
| **LH 내부 규정** | 업무지침, 시행세칙, 내규, 사규 | 로컬 벡터DB |
...
MCP 서버를 처음 실행하기 전에 LH 규정을 ChromaDB에 수집합니다. 약 10~15분 소요됩니다.

```bash
uv run lh-build-index
```
...
scripts/
└── build_index.py  # lh-build-index 엔트리포인트
```
